...
function SourceExtraction({ run, source }: { run: ReconciliationRun; source: DocumentSource }) {
  ...
  return (
    <div className="border border-[var(--border)] bg-[var(--surface-subtle)] px-4 py-3">
...
function Metric({ label, value, detail, tone = "neutral" }: { label: string; value: string; detail: string; tone?: "neutral" | "success" | "warning" }) {
  ...
  return <div className="border border-[var(--border)] bg-[var(--surface-subtle)] p-3"><div className="text-xs font-bold uppercase tracking-wide text-slate-500">{label}</div><div className={`mt-2 text-2xl font-bold ${toneClass}`}>{value}</div><div className="mt-1 text-xs text-slate-500">{detail}</div></div>;
}
