...
export function AdminProcessDetail({
  ...
  backHref = "/admin",
  embedded = false,
  showTechnicalExtractionDetails = false,
}: {
  ...
  backHref?: string;
  embedded?: boolean;
  showTechnicalExtractionDetails?: boolean;
}) {
  ...
  return <main className={embedded ? "" : "min-h-screen bg-slate-50"}>{!embedded ? <header className="border-b bg-white"><div className="mx-auto flex max-w-7xl items-center justify-between px-5 py-4"><div><div className="font-bold tracking-tight">Resultado do processo</div><div className="text-xs text-slate-500">{processCode(process.id)} · {process.profiles?.name} · {new Date(process.started_at).toLocaleString("pt-BR")}</div></div><a href={backHref} className="text-sm font-bold text-blue-600">Voltar</a></div></header> : null}<div className={`mx-auto max-w-7xl space-y-4 ${embedded ? "" : "px-5 py-8"}`}>{embedded ? <div className="app-section-heading flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between"><div><div className="mb-2 inline-flex rounded-md border border-slate-200 bg-slate-50 px-2 py-1 text-xs font-bold text-[#0f8f88]">{processCode(process.id)}</div><h1 className="text-2xl font-bold tracking-tight text-slate-950">Resultado do processo</h1><p className="mt-1 text-sm text-slate-500">{process.profiles?.name} · {new Date(process.started_at).toLocaleString("pt-BR")}</p></div><a href={backHref} className="text-sm font-bold text-[#0f8f88]">Voltar</a></div> : null}<section className="app-card flex flex-col gap-3 p-4 lg:flex-row lg:flex-wrap lg:items-center lg:justify-between"><div><div className="text-xs font-bold uppercase tracking-[0.08em] text-slate-500">Exportar conferência</div><p className="mt-1 text-sm text-slate-500">Gere um PDF completo ou somente com os itens selecionados.</p></div><div className="flex flex-col gap-2 sm:flex-row"><select aria-label="Filtro do relatório" className="min-h-10 rounded-md border border-slate-300 bg-white px-3 text-sm font-semibold text-slate-700" value={reportFilter} onChange={(event) => setReportFilter(event.target.value as typeof reportFilter)}><option value="ALL">Relatório completo</option><option value="DIVERGENCES">Somente divergências pendentes</option><option value="PENDING">Todas as pendências</option><option value="CHECKED">Somente itens conferidos</option></select><button className="inline-flex min-h-10 items-center justify-center gap-2 rounded-md bg-blue-600 px-4 text-sm font-bold text-white hover:bg-blue-700 disabled:cursor-wait disabled:bg-blue-400" onClick={exportReport} disabled={exporting}>{exporting ? <Loader2 className="h-4 w-4 animate-spin" /> : <Download className="h-4 w-4" />}{exporting ? "Gerando PDF..." : "Exportar PDF"}</button></div>{exportError ? <p className="text-sm font-semibold text-rose-600 lg:basis-full">{exportError}</p> : null}</section><section className="app-card p-4"><div className="text-xs font-bold uppercase tracking-[0.08em] text-slate-500">Documentos</div><ProcessDocumentList processId={process.id} documents={process.process_documents} /></section><ChecklistOutcomeSummary run={run} />{showTechnicalExtractionDetails ? <ExtractionQualityPanel reports={run.extractionQualityBySource} checklist={run.checklist} /> : null}<ReconciliationResultsTable results={run.results} sources={run.participatingSources} onReview={review} reviewer={currentUser} /></div></main>;
}
