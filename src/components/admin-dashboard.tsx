...
type ManagedProcess = { id: string; final_status: string; started_at: string; profiles: { name: string } | null; process_documents: Array<{ name: string }> };
type AuditEvent = {
  id: string;
  event_type: string;
  entity_type: string;
...
export function AdminDashboard() {
  ...
  async function load() { setLoading(true); const [u, p, a] = await Promise.all([fetch("/api/admin/users"), fetch("/api/processes"), fetch("/api/admin/audit")]); if (u.ok) setUsers((await u.json()).users); if (p.ok) setProcesses((await p.json()).processes); if (a.ok) setEvents((await a.json()).events); setLoading(false); }
  ...
  return <main className="min-h-screen bg-slate-50">
    ...
    <div className="mx-auto max-w-7xl space-y-8 px-5 py-8">
      ...
      <section className="border border-slate-200 bg-white"><div className="flex items-center justify-between border-b p-5"><h2 className="font-bold">Processos da equipe</h2><select className="rounded-md border px-3 py-2 text-sm" value={filter} onChange={(event) => setFilter(event.target.value)}><option value="ALL">Todos</option><option value="IN_PROGRESS">Em andamento</option><option value="PENDING_REVIEW">Pendentes</option><option value="FULLY_CHECKED">Conferidos</option><option value="FAILED">Falhas</option></select></div>
        {loading ? <div className="flex justify-center p-8"><Loader2 className="animate-spin" /></div> : <div className="divide-y">{visible.map((process) => <div key={process.id} className="grid gap-3 p-4 sm:grid-cols-[180px_1fr_150px_100px]"><div><div className="text-sm font-bold">{process.profiles?.name ?? "Usuário"}</div><div className="text-xs text-slate-500">{new Date(process.started_at).toLocaleString("pt-BR")}</div></div><div>{process.process_documents.map((document) => <div key={document.name} className="truncate text-sm text-slate-700">{document.name}</div>)}</div><div className="text-sm font-bold text-slate-600">{statusLabel(process.final_status)}</div><Link href={`/admin/processes/${process.id}`} className="text-sm font-bold text-blue-600">Ver resultado</Link></div>)}</div>}
      </section>
    </div>
  </main>;
}
