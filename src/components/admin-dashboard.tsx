...
export function AdminDashboard({ isMasterAdmin, embedded = false }: { isMasterAdmin: boolean; embedded?: boolean }) {
  ...
  return <main className={embedded ? "" : "min-h-screen bg-slate-50"}>
    {!embedded ? <header className="border-b border-slate-200 bg-white"><div className="mx-auto flex max-w-7xl items-center justify-between px-5 py-4"><div><div className="text-lg font-bold tracking-tight">ConferIA Admin</div><div className="text-xs text-slate-500">Gestão operacional</div></div><div className="flex items-center gap-4"><Link href={{ pathname: "/history" }} className="text-sm font-bold text-slate-600">Histórico</Link><Link href="/change-password" className="text-sm font-bold text-slate-600">Alterar minha senha</Link><Link href="/" className="text-sm font-bold text-blue-600">Nova conferência</Link></div></div></header> : null}
    <div className={`mx-auto max-w-7xl space-y-6 ${embedded ? "" : "px-5 py-8"}`}>
      <section><div className="app-section-heading flex items-center justify-between"><div><p className="text-xs font-bold uppercase tracking-[0.12em] text-blue-700">Painel administrativo</p><h1 className="mt-1 text-2xl font-bold tracking-tight">Visão geral</h1><p className="mt-1 text-sm text-slate-500">{isMasterAdmin ? "Atividades, segurança e resultados de todas as organizações." : "Atividades e resultados da sua organização."}</p></div><button title="Atualizar" onClick={load} className="rounded-md border border-slate-200 bg-white p-2 text-slate-600 shadow-sm hover:border-blue-300 hover:text-blue-700"><RefreshCw className={`h-4 w-4 ${loading ? "animate-spin" : ""}`} /></button></div><div className="mt-4 grid gap-2 sm:grid-cols-5"><Metric label="Documentos analisados" value={documentsAnalyzed} /><Metric label="Documentos em análise" value={documentsInProgress} /><Metric label="Documentos cadastrados" value={documentsTotal} /><Metric label="Processos em andamento" value={inProgressProcesses.length} /><Metric label="Alertas operacionais" value={operationalAlerts.length} alert={operationalAlerts.length > 0} /></div><div className="mt-2 grid gap-2 sm:grid-cols-4"><Metric label="Com pendências" value={count("PENDING_REVIEW")} /><Metric label="Conferidos" value={count("FULLY_CHECKED")} /><Metric label="Falhas" value={count("FAILED")} /><Metric label="Usuários ativos" value={users.filter((user) => user.active).length} /></div>{isMasterAdmin ? <div className="mt-2 grid gap-2 sm:grid-cols-4"><Metric label="Campos críticos ausentes" value={qualityMetrics.missingCritical} alert={qualityMetrics.missingCritical > 0} /><Metric label="Baixa confiança crítica" value={qualityMetrics.lowConfidence} alert={qualityMetrics.lowConfidence > 0} /><Metric label="Conflitos internos críticos" value={qualityMetrics.ambiguous} alert={qualityMetrics.ambiguous > 0} /><Metric label="Recuperados automaticamente" value={qualityMetrics.recovered + qualityMetrics.deterministic} /></div> : null}</section>
      {operationalAlerts.length ? <section className="app-card border-amber-300 bg-amber-50">
        <div className="flex items-start gap-3 border-b border-amber-200 p-4">
      ...
      {isMasterAdmin && securityAlerts.length ? <section className="app-card border-rose-300 bg-rose-50">
        <div className="flex items-start gap-3 border-b border-rose-200 p-4">
      ...
      <section className="app-card overflow-hidden"><div className="flex flex-col gap-3 border-b p-4 sm:flex-row sm:items-center sm:justify-between"><div><h2 className="font-bold">Processos da equipe</h2><p className="mt-1 text-xs text-slate-500">Acompanhe documentos, duração, situação e próximos passos.</p></div><select className="rounded-md border px-3 py-2 text-sm" value={filter} onChange={(event) => setFilter(event.target.value)}><option value="ALL">Todos</option><option value="ANOMALY">Com alerta</option><option value="IN_PROGRESS">Em andamento</option><option value="PENDING_REVIEW">Pendentes</option><option value="FULLY_CHECKED">Conferidos</option><option value="FAILED">Falhas</option></select></div>
...
function Metric({ label, value, alert = false }: { label: string; value: number; alert?: boolean }) { return <div className={`app-card p-3 ${alert ? "border-amber-300 bg-amber-50" : ""}`}><div className={`text-2xl font-bold tracking-tight ${alert ? "text-amber-800" : ""}`}>{value}</div><div className={`mt-1 text-xs font-semibold ${alert ? "text-amber-800" : "text-slate-500"}`}>{label}</div></div>; }
